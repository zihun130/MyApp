...
public class RdRecyclerAdapter extends RecyclerView.Adapter<RdRecyclerAdapter.BaseHolder> {
    ...
    private void setListener(BaseHolder holder, final RdBean.ListBean listBean) {
        ...
        //设置分享的监听
        holder.llItemShare.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                showShare(listBean);
            }
    ...
    private void showShare(RdBean.ListBean listBean) {
        ...
        oks.setText(listBean.getText());
        // imagePath是图片的本地路径，Linked-In以外的平台都支持此参数
        oks.setImagePath("/sdcard/test.jpg");//确保SDcard下面存在此张图片
        // url仅在微信（包括好友和朋友圈）中使用
        oks.setUrl(listBean.getShare_url());
        // comment是我对这条分享的评论，仅在人人网和QQ空间使用
        oks.setComment("我是测试评论文本");
        // site是分享此内容的网站名称，仅在QQ空间使用
        oks.setSite(context.getString(R.string.app_name));
        // siteUrl是分享此内容的网站地址，仅在QQ空间使用
        oks.setSiteUrl(listBean.getShare_url());
        // 启动分享GUI
        oks.show(context);
    }
